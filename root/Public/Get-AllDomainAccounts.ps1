Function Get-AllDomainAccounts()
{
    ...
    foreach($domain in $Global:dbUser)
    {
        If($domain.isChecked -eq $true){
            ...
            RunspacePing -totalJobs $dbUser.Count -userListView $Global:userListView -dbUser $Global:dbUser -syncHash $syncHash -samAccountNAme "cberg" -dc $domain.dc -domainName $domain.domainName -whatIf $false      
